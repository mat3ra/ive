 ...
 *
 * The compute form asks for nodes, cores and a walltime and says nothing about
 * what they add up to. The same arithmetic is needed wherever that shows: the
 * estimate panel here, and in job-designer the context strip's estimate chip and
 * the submit preflight's budget check. Surfaces disagreeing about what a job
 * costs would be worse than none of them saying anything, so it lives here —
 * beside the form that holds the live values — and job-designer imports it.
 *
