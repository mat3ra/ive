 ...
 */
export function fieldNameFromFieldId(fieldId) {
    if (!fieldId || !fieldId.startsWith(RJSF_FIELD_ID_PREFIX))
        return null;
 ...
 */
export function withTouchedField(touchedFields, fieldId) {
...
export function shouldShowFieldError({ fieldName, touchedFields, showAllErrors, }) {
    if (showAllErrors)
        return true;
    if (!fieldName)
        return false;
