...
`github:mat3ra/ive#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/ive/releases/download/wip-<short-sha>/ive.tgz
```
...
```bash
npm run mat3ra:install -- ive wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/ive@<release-url>` with
