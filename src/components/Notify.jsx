...
import AccountCard from "@exabyte-io/cove.js/dist/mui-composed/components/account/AccountCard";
...
class Notify extends React.Component {
    ...
    selectNotifyAccount = (event) => {
        ...
        if (!selectedUsers.length) {
            ...
            if (isAbort) updatedNotify += "a";
            if (isBegin) updatedNotify += "b";
            if (isEnd) updatedNotify += "e";
            ...
            <Paper sx={{ mb: 3, p: 3 }}>
                            ...
                            {accountUsers.map((item) => (
                                <MenuItem
                                    ...
                                    <Checkbox
                                        checked={
                                            !!selectedUsers.find(
                                                (user) => user.entity.email === item.entity.email,
                                            )
                                        }
                                    />
                                    <AccountCard
                                        account={item.account.entity}
                                        subtitle={item.entity.email}
                                    />
                                </MenuItem>
                            ))}
                        </Select>
                ...
                <Box sx={{ display: "flex", flexDirection: "column" }} mt={2}>
                    ...
                    <Typography variant="caption" mb={1}>
                        Choose events to trigger notifications
                    </Typography>
                    <ButtonGroup size="small">
                        <Button
                            ...
                            variant={isBegin ? "contained" : "outlined"}
                            disabled={!editable || !isSelectedUsers}
                            id="email-opt-b"
                            onClick={() => this.toggleOption(IS_BEGIN)}>
                            Started
                        </Button>
                        <Button
                            className={setClass({
                            ...
                            variant={isAbort ? "contained" : "outlined"}
                            disabled={!editable || !isSelectedUsers}
                            id="email-opt-a"
                            onClick={() => this.toggleOption(IS_ABORT)}>
                            Aborted
                        </Button>
                        <Button
                            className={setClass({
                            ...
                            disabled={!editable || !isSelectedUsers}
                            id="email-opt-e"
                            onClick={() => this.toggleOption(IS_END)}>
                            Ended
                        </Button>
                    </ButtonGroup>
                </Box>
